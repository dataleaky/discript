...
class AuditLog extends Base_1.default {
    constructor(data) {
        ...
        this.webhooks = data.webhooks === undefined ? data.webhooks : (() => {
            const collection = new Collection_1.default();
            for (const object of data.webhooks) {
                const subject = new Webhook_1.default({ client: this._client, ...object });
                collection.set(subject.flake.id, subject);
            }
            return collection;
        })();
        ...
        this.auditLogEntries = data.audit_log_entries === undefined ? data.audit_log_entries : (() => {
            const collection = new Collection_1.default();
            for (const object of data.audit_log_entries) {
                const subject = new AuditLogEntry({ client: this._client, ...object });
                collection.set(subject.flake.id, subject);
            }
            return collection;
        })();
        this.integrations = data.integrations === undefined ? data.integrations : (() => {
            ...
            for (const object of data.integrations) {
                const subject = new Integration_1.default({ client: this._client, ...object });
                collection.set(subject.flake.id, subject);
            }
            return collection;
        })();
...
class AuditLogEntry extends Base_1.default {
    constructor(data) {
        super();
        this._client = data.client;
        this.targetFlake = data.target_id === undefined || data.target_id === null ? data.target_id : new Flake_1.default(data.target_id);
        this.changes = data.changes === undefined ? data.changes : data.changes.map(element => new AuditLogChange({ client: this._client, ...element }));
...
class OptionalAuditEntryInfo extends Base_1.default {
    constructor(data) {
        ...
        this.flake = data.id === undefined ? data.id : new Flake_1.default(data.id);
        this.type = data.type === undefined ? data.type : Number(data.type);
...
class AuditLogChange extends Base_1.default {
    constructor(data) {
        ...
        this._client = data.client;
        this.key = change[0];
        this.newValue = data.new_value === undefined ? data.new_value : change[1](data.new_value, this._client);
        this.oldValue = data.old_value === undefined ? data.old_value : change[1](data.old_value, this._client);
    }
}
