...
class Integration extends Base_1.default {
    constructor(data) {
        ...
        this.account = data.account === undefined ? data.account : new IntegrationAccount({ _client: this._client, ...data.account });
        ...
        this.app = data.application === undefined ? data.application : new IntegrationApp({ _client: this._client, ...data.application });
    }
    get syncedDate() {
...
class IntegrationApp extends Base_1.default {
    constructor(data) {
        ...
        this.bot = data.bot === undefined ? data.bot : new User_1.default({ _client: this._client, ...data.bot });
    }
    get app() {
    ...
    constructor(data) {
        ...
        this.integrations = data.integrations === undefined ? data.integrations : (() => {
            ...
            for (const object of data.integrations) {
                const subject = new Integration({ _client: this._client, ...object });
