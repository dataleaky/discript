...
class Message extends Base_1.default {
    constructor(data) {
        ...
        this.author = data.author === undefined ? data.author : this.webhookFlake === undefined ? new User_1.default({ client: this._client, ...data.author }) : new Webhook_1.default({ client: this._client, ...data.author });
        this.member = data.member === undefined ? data.member : new Server_1.ServerMember({ client: this._client, ...data.member });
        ...
        this.mentionUsers = data.mentions === undefined ? data.mentions : (() => {
            ...
            for (const object of data.mentions) {
                const subject = new User_1.default({ client: this._client, ...object });
                subject.member = new Server_1.ServerMember({ client: this._client, ...object.member });
        ...
        this.mentionChannels = data.mention_channels === undefined ? data.mention_channels : (() => {
            ...
            for (const object of data.mention_channels) {
                const subject = new ChannelMention({ client: this._client, ...object });
                collection.set(subject.flake.id, subject);
            }
            return collection;
        })();
        ...
        this.reactions = data.reactions === undefined ? data.reactions : data.reactions.map(element => new Reaction({ client: this._client, ...element }));
        this.nonce = data.nonce;
        this.isPinned = data.pinned;
        this.type = data.type;
        this.activity = data.activity === undefined ? data.activity : new MessageActivity({ client: this._client, ...data.activity });
        this.app = data.application === undefined ? data.application : new MessageApp({ client: this._client, ...data.application });
        this.reference = data.message_reference === undefined ? data.message_reference : new MessageReference(data.message_reference);
        this.flags = data.flags;
        this.replyMessage = data.referenced_message === undefined || data.referenced_message === null ? data.referenced_message : new Message({ client: this._client, ...data.referenced_message });
...
class MessageApp extends Base_1.default {
    ...
    getAssetURL(params) {
        return this.coverImage === undefined ? null : this._client.getAppAssetURL({ ...params, appID: String(this.flake.id), id: this.coverImage });
    }
}
...
class Reaction extends Base_1.default {
    constructor(data) {
        ...
        this.isMe = data.me;
        this.emoji = data.emoji === undefined ? data.emoji : new Server_1.Emoji({ client: this._client, ...data.emoji });
    }
}
