...
class Presence extends Base_1.default {
    constructor(data) {
        ...
        this.activities = data.activities === undefined ? data.activities : data.activities.map(element => new Activity({ client: this._client, ...element }));
...
class Activity extends Base_1.default {
    constructor(data) {
        ...
        this.emoji = data.emoji === undefined || data.emoji === null ? data.emoji : new ActivityEmoji({ client: this._client, ...data.emoji });
        this.party = data.party === undefined ? data.party : new ActivityParty({ client: this._client, ...data.party });
        this.assets = data.assets === undefined ? data.assets : new ActivityAssets({ client: this._client, ...data.assets, appFlake: this.appFlake });
...
class ActivityEmoji extends Base_1.default {
    ...
    getEmojiURL(params) {
        return this.flake === undefined ? undefined : this._client.getCustomEmojiURL({ ...params, animated: Boolean(this.isAnimated), id: String(this.flake.id) });
    }
}
...
class ActivityAssets extends Base_1.default {
    ...
    getLargeImageURL(params) {
        return this._appFlake === undefined ? this._appFlake : this.largeImage === undefined ? this.largeImage : this._client.getAppAssetURL({ ...params, id: this.largeImage, appID: String(this._appFlake.id) });
    }
    getSmallImageURL(params) {
        return this._appFlake === undefined ? this._appFlake : this.smallImage === undefined ? this.smallImage : this._client.getAppAssetURL({ ...params, size: params.size, id: this.smallImage, appID: String(this._appFlake.id) });
    }
}
