...
class Client extends Base.default {
  constructor(options) {
    super();
    this.options = {
      token: options.token,
      properties: options.properties === undefined ? {
        $os: process.platform,
        $browser: packageJSON.name,
        $device: packageJSON.name
      } : options.properties,
      ...
      defaultEndpoints: options.defaultEndpoints === undefined ? {
        ...
        Gateway: () => `${this.handler.baseURL}/gateway`,
        GatewayBot: () => `${this.handler.baseURL}/gateway/bot`,
        GuildAuditLog: (serverID) => `${this.handler.baseURL}/guilds/${serverID}/audit-logs`
      } : options.defaultEndpoints,
      userAgent: options.userAgent === undefined ? `DiscordBot (${packageJSON.homepage}, ${packageJSON.version})` : options.userAgent,
    ...
    this.handler = new RequestHandler.default(this);
    this.servers = new Collection.default();
    this.channels = new Collection.default();
    this.user = User.default.prototype;
    this.startTime = 0;
    this.ping = Infinity;
  }
  ;
  ...
  async getApplicationInformation(options) {
    return new Application.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/oauth2/applications/${options.userID === undefined ? '@me' : options.userID}` }) });
  }
  ;
  async getAuditLog(params) {
    return new AuditLog.default({ client: this, ...await this.handler.request({ method: 'GET', url: this.options.defaultEndpoints.GuildAuditLog(params.serverID), body: { user_id: params.userID, action_type: params.actionType, before: params.before, limit: params.limit } }) });
  }
  ;
  async getChannel(params) {
    return new Channel.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/channels/${params.channelID}` }) });
  }
  ;
  async updateChannel(options) {
    return new Channel.default({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/channels/${options.channelID}`, body: { name: options.options.name, type: options.options.type, position: options.options.position, topic: options.options.topic, nsfw: options.options.nsfw, rate_limit_per_user: options.options.rateLimitPerUser, bitrate: options.options.bitrate, user_limit: options.options.userLimit, permission_overwrites: options.options.permissionOverwrites, parent_id: options.options.parentID, reason: options.reason } }) });
  }
  ;
  async deleteServerChannel(options) {
    return new Channel.default({ client: this, ...await this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/channels/${options.channelID}`, body: { reason: options.reason } }) });
  }
  ;
  async closeDMChannel(options) {
    return new Channel.default({ client: this, ...await this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/channels/${options.channelID}` }) });
  }
  ;
  async getMessages(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Message.default({ client: this, ...object });
  ...
  async getMessage(options) {
    return new Message.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/channels/${options.channelID}/messages/${options.messageID}` }) });
  }
  ;
  async createMessage(options) {
    return new Message.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/channels/${options.channelID}/messages`, body: { content: options.options.content, nonce: options.options.nonce, tts: options.options.tts, embed: options.options.embed, payload_JSON: options.options.payloadJSON, allowed_mentions: options.options.allowedMentions }, file: options.options.file }) });
  }
  ;
  async crosspostMessage(options) {
    return new Message.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/channels/${options.channelID}/messages/${options.messageID}/crosspost` }) });
  }
  ;
  async createReaction(options) {
    this.handler.request({ method: 'PUT', url: `${this.handler.baseURL}/channels/${options.channelID}/messages/${options.messageID}/reactions/${options.emoji}/${options.userID === undefined ? '@me' : options.userID}` });
  }
  ;
  ...
  async getReactions(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new User.default({ client: this, ...object });
      collection.set(subject.flake.id, subject);
    }
    ;
    return collection;
  ...
  async updateMessage(options) {
    return new Message.default({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/channels/${options.channelID}/messages/${options.messageID}`, body: { content: options.options.content, embed: options.options.embed, flags: options.options.flags } }) });
  }
  ;
  async deleteMessage(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/channels/${options.channelID}/messages/${options.messageID}`, body: { reason: options.reason } });
  }
  ;
  async deleteBulkMessages(options) {
    this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/channels/${options.channelID}/messages/bulk-delete`, body: { messages: options.options.messages, reason: options.reason } });
  }
  ;
  async updateChannelPermissions(options) {
    this.handler.request({ method: 'PUT', url: `${this.handler.baseURL}/channels/${options.channelID}/permissions/${options.overwriteID}`, body: { allow: options.options.allow, deny: options.options.deny, type: options.options.type, reason: options.reason } });
  }
  ;
  async getChannelInvites(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Invite.InviteMetadata({ client: this, ...object });
  ...
  async createChannelInvite(options) {
    return new Invite.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/channels/${options.channelID}/invites`, body: { max_age: options.options.maxAge, max_uses: options.options.maxUses, temporary: options.options.temporary, unique: options.options.unique, target_user: options.options.targetUser, target_user_type: options.options.targetUserType, reason: options.reason } }) });
  }
  ;
  async deleteChannelPermission(options) {
    this.handler.request({ method: 'PUT', url: `${this.handler.baseURL}/channels/${options.channelID}/permissions/${options.overwriteID}`, body: { reason: options.reason } });
  }
  ;
  async followNewsChannel(options) {
    return new Channel.FollowedChannel(await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/channels/${options.channelID}/followers`, body: { webhook_channel_id: options.options.webhookChannelID } }));
  }
  ;
  async triggerTypingIndicator(options) {
    this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/channels/${options.channelID}/typing` });
  }
  ;
  ...
  async createPinnedChannelMessage(options) {
    this.handler.request({ method: 'PUT', url: `${this.handler.baseURL}/channels/${options.channelID}/pins/${options.messageID}`, body: { reason: options.reason } });
  }
  ;
  async deletePinnedChannelMessage(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/channels/${options.channelID}/pins/${options.messageID}`, body: { reason: options.reason } });
  }
  ;
  async createGroupDMRecipient(options) {
    this.handler.request({ method: 'PUT', url: `${this.handler.baseURL}/channels/${options.channelID}/recipients/${options.userID}`, body: { access_token: options.options.accessToken, nick: options.options.nick } });
  }
  ;
  async deleteGroupDMRecipient(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/channels/${options.channelID}/recipients/${options.userID}` });
  }
  ;
  async getServerEmojis(options) {
    const data = await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.serverID}/emojis` });
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Server.Emoji({ client: this, ...object });
  ...
  async getServerEmoji(options) {
    return new Server.Emoji({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.guildID}/emojis/${options.emojiID}` }) });
  }
  ;
  async createServerEmoji(options) {
    return new Server.Emoji({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/guilds/${options.guildID}/emojis`, body: { name: options.options.name, image: options.options.image, roles: options.options.roles, reason: options.reason } }) });
  }
  ;
  async updateServerEmoji(options) {
    return new Server.Emoji({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/guilds/${options.guildID}/emojis/${options.emojiID}`, body: { name: options.options.name, image: options.options.image, reason: options.reason } }) });
  }
  ;
  async deleteServerEmoji(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/guilds/${options.guildID}/emojis/${options.emojiID}`, body: { reason: options.reason } });
  }
  ;
  async createServer(options) {
    return new Server.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/guilds`, body: { name: options.options.name, region: options.options.region, icon: options.options.icon, verification_level: options.options.verificationLevel, default_message_notifications: options.options.defaultMessageNotifications, explicit_content_filter: options.options.explicitContentFilter, roles: options.options.roles, channels: options.options.channels, afk_channel_id: options.options.afkChannelID, afk_timeout: options.options.afkTimeout, system_channel_id: options.options.systemChannelID } }) });
  }
  ;
  async getServer(options) {
    return new Server.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.guildID}`, body: { with_counts: options.options.withCounts } }) });
  }
  ;
  async getServerPreview(options) {
    return new Server.ServerPreview({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.guildID}/preview` }) });
  }
  ;
  async updateServer(options) {
    return new Server.default({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/guilds/${options.guildID}`, body: { name: options.options.name, region: options.options.region, verification_level: options.options.verificationLevel, default_message_notifications: options.options.defaultMessageNotifications, explicit_content_filter: options.options.explicitContentFilter, afk_channel_id: options.options.afkChannelID, afk_timeout: options.options.afkTimeout, icon: options.options.icon, owner_id: options.options.ownerID, splash: options.options.splash, banner: options.options.banner, system_channel_id: options.options.systemChannelID, rules_channel_id: options.options.rulesChannelID, public_updates_channel_id: options.options.publicUpdatesChannelID, preferred_locale: options.options.preferredLocale, reason: options.reason } }) });
  }
  ;
  async deleteServer(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/guilds/${options.guildID}` });
  }
  ;
  async getServerChannels(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Channel.default({ client: this, ...object });
  ...
  async getServerMembers(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Server.ServerMember({ client: this, ...object });
  ...
  async createServerMember(options) {
    return new Server.ServerMember({ client: this, ...await this.handler.request({ method: 'PUT', url: `${this.handler.baseURL}/guilds/${options.guildID}/members/${options.userID}`, body: { access_token: options.options.accessToken, nick: options.options.nick, roles: options.options.roles, mute: options.options.mute, deaf: options.options.deaf } }) });
  }
  ;
  async updateServerMember(options) {
    this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/guilds/${options.guildID}/members/${options.userID}`, body: { nick: options.options.nick, roles: options.options.roles, mute: options.options.mute, deaf: options.options.deaf, channel_id: options.options.channelID, reason: options.reason } });
  }
  ;
  async updateUserNick(options) {
    this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/guilds/${options.guildID}/members/${options.userID === undefined ? '@me' : options.userID}/nick`, body: { nick: options.options.nick } });
  }
  ;
  async createServerMemberRole(options) {
    this.handler.request({ method: 'PUT', url: `${this.handler.baseURL}/guilds/${options.guildID}/members/${options.userID}/roles/${options.roleID}`, body: { reason: options.reason } });
  }
  ;
  async deleteServerMemberRole(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/guilds/${options.guildID}/members/${options.userID}/roles/${options.roleID}`, body: { reason: options.reason } });
  }
  ;
  async deleteServerMember(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/guilds/${options.guildID}/members/${options.userID}`, body: { reason: options.reason } });
  }
  ;
  async getServerBans(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Server.Ban({ client: this, ...object });
      collection.set(subject.user.flake.id, subject);
    }
    ;
    return collection;
  ...
  async getServerBan(options) {
    return new Server.Ban({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.guildID}/bans/${options.userID}` }) });
  }
  ;
  async createServerBan(options) {
    this.handler.request({ method: 'PUT', url: `${this.handler.baseURL}/guilds/${options.guildID}/bans/${options.userID}`, body: { delete_message_days: options.options.deleteMessageDays, reason: options.options.reason } });
  }
  ;
  async deleteServerBan(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/guilds/${options.guildID}/bans/${options.userID}`, body: { reason: options.reason } });
  }
  ;
  async getServerRoles(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Server.Role(object);
      collection.set(subject.flake.id, subject);
    }
    ;
    return collection;
  ...
  async updateServerRole(options) {
    return new Server.Role(await this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/guilds/${options.guildID}/roles/${options.roleID}`, body: { name: options.options.name, permissions: options.options.permissions, color: options.options.color, hoist: options.options.hoist, mentionable: options.options.mentionable, reason: options.reason } }));
  }
  ;
  async deleteServerRole(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/guilds/${options.guildID}/roles/${options.roleID}`, body: { reason: options.reason } });
  }
  ;
  async getServerPruneCount(options) {
    this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.guildID}/prune`, body: { days: options.options.days, include_roles: options.options.includeRoles } });
  }
  ;
  async beginServerPrune(options) {
    this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/guilds/${options.guildID}/prune`, body: { days: options.options.days, compute_prune_count: options.options.computePruneCount, include_roles: options.options.includeRoles, reason: options.reason } });
  }
  ;
  async getServerVoiceRegions(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Server.VoiceRegion(object);
  ...
  async getServerIntegrations(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Integration.default({ client: this, ...object });
      collection.set(subject.flake.id, subject);
    }
    ;
    return collection;
  ...
  async createServerIntegration(options) {
    this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/guilds/${options.guildID}/integrations`, body: { type: options.options.type, id: options.options.id, reason: options.reason } });
  }
  ;
  async updateServerIntegration(options) {
    this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/guilds/${options.guildID}/integrations/${options.integrationID}`, body: { expire_behavior: options.options.expireBehavior, expire_grace_period: options.options.expireGracePeriod, enable_emoticons: options.options.enableEmoticons, reason: options.reason } });
  }
  ;
  async deleteServerIntegration(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/guilds/${options.guildID}/integrations/${options.integrationID}`, body: { reason: options.reason } });
  }
  ;
  async syncServerIntegration(options) {
    this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/guilds/${options.guildID}/integrations/${options.integrationID}/sync` });
  }
  ;
  async getServerWidgetSettings(options) {
    return new Server.ServerWidget(await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.guildID}/widget` }));
  }
  ;
  async updateServerWidget(options) {
    return new Server.ServerWidget(await this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/guilds/${options.guildID}/widget`, body: options }));
  }
  ;
  async getServerWidget(options) {
    this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.guildID}/widget.json` });
  }
  ;
  async getServerVanityURL(options) {
    return new Invite.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guilds/${options.guildID}/vanity-url` }) });
  }
  ;
  ...
  async getInvite(options) {
    return new Invite.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/invites/${options.inviteCode}`, body: { with_counts: options.options.withCounts } }) });
  }
  ;
  async deleteInvite(options) {
    return new Invite.default({ client: this, ...await this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/invites/${options.inviteCode}`, body: { reason: options.reason } }) });
  }
  ;
  async getTemplate(options) {
    return new Server.Template({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}guilds/templates/${options.templateCode}` }) });
  }
  ;
  async createServerFromTemplate(options) {
    return new Server.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}guilds/templates/${options.templateCode}`, body: { name: options.options.name, icon: options.options.icon } }) });
  }
  ;
  ...
  async getUser(options) {
    return new User.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/users/${options.userID === undefined ? '@me' : options.userID}` }) });
  }
  ;
  async updateUser(options) {
    return new User.default({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.handler.baseURL}/users/${options.userID === undefined ? '@me' : options.userID}`, body: { username: options.options.username, avatar: options.options.avatar } }) });
  }
  ;
  async getServers(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Server.default({ client: this, ...object });
      collection.set(subject.flake.id, subject);
    }
    ;
    return collection;
  ...
  async leaveServer(options) {
    this.handler.request({ method: 'DELETE', url: `${this.handler.baseURL}/users/${options.userID === undefined ? '@me' : options.userID}/guilds/${options.guildID}` });
  }
  ;
  ...
  async createDM(options) {
    return new Channel.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/users/${options.userID === undefined ? '@me' : options.userID}/channels`, body: { recipient_id: options.options.recipientID } }) });
  }
  ;
  async createGroupDM(options) {
    return new Channel.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/users/${options.userID === undefined ? '@me' : options.userID}/channels`, body: { access_tokens: options.options.accessTokens, nicks: options.options.nicks } }) });
  }
  ;
  async getConnections(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Integration.Connection({ client: this, ...object });
      collection.set(subject.flake.id, subject);
    }
    ;
    return collection;
  ...
  async createChannelWebhook(options) {
    return new Webhook.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.handler.baseURL}/channels/${options.channelID}/webhooks`, body: { name: options.options.name, avatar: options.options.avatar, reason: options.reason } }) });
  }
  ;
  async getChannelWebhooks(options) {
    ...
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Webhook.default({ client: this, ...object });
      collection.set(subject.flake.id, subject);
    }
    ;
    return collection;
  ...
  async getServerWebhooks(options) {
    const data = await this.handler.request({ method: 'GET', url: `${this.handler.baseURL}/guild/${options.serverID}/webhooks` });
    const collection = new Collection.default();
    for (const object of data) {
      const subject = new Webhook.default({ client: this, ...object });
      collection.set(subject.flake.id, subject);
  ...
  async getWebhook(options) {
    return new Webhook.default({ client: this, ...await this.handler.request({ method: 'GET', url: (options.webhookToken === undefined ? `${this.handler.baseURL}/webhooks/${options.webhookID}` : `${this.handler.baseURL}/webhooks/${options.webhookID}/${options.webhookToken}`) }) });
  }
  ;
  async updateWebhook(options) {
    return new Webhook.default({ client: this, ...await this.handler.request({ method: 'PATCH', url: (options.webhookToken === undefined ? `${this.handler.baseURL}/webhooks/${options.webhookID}` : `${this.handler.baseURL}/webhooks/${options.webhookID}/${options.webhookToken}`), body: { name: options.options.name, avatar: options.options.avatar, channel_id: options.options.channelID, reason: options.reason } }) });
  }
  ;
  async deleteWebhook(options) {
    this.handler.request({ method: 'DELETE', url: (options.webhookToken === undefined ? `${this.handler.baseURL}/webhooks/${options.webhookID}` : `${this.handler.baseURL}/webhooks/${options.webhookID}/${options.webhookToken}`), body: { reason: options.reason } });
  }
  ;
