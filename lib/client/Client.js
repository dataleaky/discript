...
const DefaultEndpoints = {
    ...
    ServerMemberVerification: (base, serverID) => `${base}/guilds/${serverID}/member-verification`,
    ...
    OAuth2: (base, userID) => `${base}/oauth2/${userID}`,
...
class Client extends Base_1.default {
    constructor(options) {
        ...
        this.options = {
            ...
            properties: options.properties === undefined ? {
                $os: process.platform,
                $browser: PackageData.name,
                $device: PackageData.name
            } : options.properties,
            ...
            userAgent: options.userAgent === undefined ? `DiscordBot (${PackageData.homepage}, ${PackageData.version})` : options.userAgent,
    ...
    async getServerMembershipScreeningForm(params) {
        return new Server_1.MembershipScreening(await this.handler.request({ method: 'GET', url: this.options.defaultEndpoints.ServerMemberVerification(this.options.defaultURLs.base, params.serverID) }));
    }
    async editServerMembershipScreeningForm(params) {
        return new Server_1.MembershipScreening(await this.handler.request({ method: 'PATCH', url: this.options.defaultEndpoints.ServerMemberVerification(this.options.defaultURLs.base, params.serverID), body: { enabled: params.isEnabled, form_fields: params.formFields, description: params.description } }));
    }
    ...
    async getAuthorization(params) {
        const object = await this.handler.request({ method: 'GET', url: this.options.defaultEndpoints.OAuth2(this.options.defaultURLs.base, params.userID === undefined ? '@me' : params.userID) });
        return { app: new App_1.default({ _client: this, ...object.application }), expires: Date.parse(object.expires), scopes: object.scopes, user: object.user === undefined ? object.user : new User_1.default({ _client: this, ...object.user }) };
    }
