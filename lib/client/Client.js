...
class Client extends Base_1.default {
    constructor(options) {
        ...
        this.options = {
            ...
            defaultEndpoints: options.defaultEndpoints === undefined ? {
                ...
                CDNAppIcon: (appID, icon, format) => `${this.options.defaultURLs.cdn}/app-icons/${appID}/${icon}.${format}`,
                CDNAppAsset: (appID, id, format) => `${this.options.defaultURLs.cdn}/app-assets/${appID}/${id}.${format}`,
                ...
                ServerAuditLog: (serverID) => `${this.options.defaultURLs.base}/guilds/${serverID}/audit-logs`
                // Authorization: '/oauth2/authorize',
                // Token: '/oauth2/token',
                //   TokenRevocation: '/oauth2/token/revoke',
    ...
    getAppIconURL(params) {
        return this.options.defaultEndpoints.CDNAppIcon(params.appID, params.icon, params.format === undefined || params.format === 'auto' || params.format === 'gif' ? this.options.defaultImageFormat : params.format) + (params.size === undefined ? `?size=${params.size === 'auto' ? this.options.defaultImageSize : params.size}` : '');
    }
    getAppAssetURL(params) {
        return this.options.defaultEndpoints.CDNAppAsset(params.appID, params.id, params.format === undefined || params.format === 'auto' || params.format === 'gif' ? this.options.defaultImageFormat : params.format) + (params.size === undefined ? `?size=${params.size === 'auto' ? this.options.defaultImageSize : params.size}` : '');
    }
    ...
    async getGateway() {
        return new Gateway(await this.handler.request({ method: 'GET', url: this.options.defaultEndpoints.Gateway() }));
    }
    async getGatewayBot() {
        return new Gateway(await this.handler.request({ method: 'GET', url: this.options.defaultEndpoints.GatewayBot() }));
    }
    async getAppInformation(options) {
        return new App_1.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/oauth2/applications/${options.userID === undefined ? '@me' : options.userID}` }) });
    }
    async getAuditLog(params) {
        return new AuditLog_1.default({ client: this, ...await this.handler.request({ method: 'GET', url: this.options.defaultEndpoints.ServerAuditLog(params.serverID), body: { user_id: params.userID, action_type: params.actionType, before: params.before, limit: params.limit } }) });
    }
    ...
    async followNewsChannel(options) {
        return new Channel_1.FollowedChannel({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.options.defaultURLs.base}/channels/${options.channelID}/followers`, body: { webhook_channel_id: options.options.webhookChannelID } }) });
    }
    ...
    async getServerEmoji(options) {
        return new Server_1.Emoji({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/emojis/${options.emojiID}` }) });
    }
    async createServerEmoji(options) {
        return new Server_1.Emoji({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/emojis`, body: { name: options.options.name, image: options.options.image, roles: options.options.roles, reason: options.reason } }) });
    }
    async updateServerEmoji(options) {
        return new Server_1.Emoji({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/emojis/${options.emojiID}`, body: { name: options.options.name, image: options.options.image, reason: options.reason } }) });
    }
    async deleteServerEmoji(options) {
        this.handler.request({ method: 'DELETE', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/emojis/${options.emojiID}`, body: { reason: options.reason } });
    }
    ...
    async getServer(options) {
        return new Server_1.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}`, body: { with_counts: options.options.withCounts } }) });
    }
    async getServerPreview(options) {
        return new Server_1.ServerPreview({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/preview` }) });
    }
    async updateServer(options) {
        return new Server_1.default({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}`, body: { name: options.options.name, region: options.options.region, verification_level: options.options.verificationLevel, default_message_notifications: options.options.defaultMessageNotifications, explicit_content_filter: options.options.explicitContentFilter, afk_channel_id: options.options.afkChannelID, afk_timeout: options.options.afkTimeout, icon: options.options.icon, owner_id: options.options.ownerID, splash: options.options.splash, banner: options.options.banner, system_channel_id: options.options.systemChannelID, rules_channel_id: options.options.rulesChannelID, public_updates_channel_id: options.options.publicUpdatesChannelID, preferred_locale: options.options.preferredLocale, reason: options.reason } }) });
    }
    async deleteServer(options) {
        this.handler.request({ method: 'DELETE', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}` });
    }
    async getServerChannels(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/channels` });
        const collection = new Collection_1.default();
        for (const object of data) {
    ...
    async createServerChannel(options) {
        return new Channel_1.default({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/channels`, body: { name: options.options.name, type: options.options.type, topic: options.options.topic, bitrate: options.options.bitrate, user_limit: options.options.userLimit, rate_limit_per_user: options.options.rateLimitPerUser, position: options.options.position, permission_overwrites: options.options.permissionOverwrites, parent_id: options.options.parentID, nsfw: options.options.nsfw, reason: options.reason } }) });
    }
    async updateServerChannelPositions(options) {
        this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/channels`, body: { id: options.options.id, position: options.options.position } });
    }
    async getServerMember(options) {
        return new Server_1.ServerMember({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/members/${options.userID}` }) });
    }
    async getServerMembers(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/members`, body: { after: options.options.after, limit: options.options.limit } });
        const collection = new Collection_1.default();
        for (const object of data) {
    ...
    async createServerMember(options) {
        return new Server_1.ServerMember({ client: this, ...await this.handler.request({ method: 'PUT', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/members/${options.userID}`, body: { access_token: options.options.accessToken, nick: options.options.nick, roles: options.options.roles, mute: options.options.mute, deaf: options.options.deaf } }) });
    }
    async updateServerMember(options) {
        this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/members/${options.userID}`, body: { nick: options.options.nick, roles: options.options.roles, mute: options.options.mute, deaf: options.options.deaf, channel_id: options.options.channelID, reason: options.reason } });
    }
    async updateUserNick(options) {
        this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/members/${options.userID === undefined ? '@me' : options.userID}/nick`, body: { nick: options.options.nick } });
    }
    async createServerMemberRole(options) {
        this.handler.request({ method: 'PUT', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/members/${options.userID}/roles/${options.roleID}`, body: { reason: options.reason } });
    }
    async deleteServerMemberRole(options) {
        this.handler.request({ method: 'DELETE', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/members/${options.userID}/roles/${options.roleID}`, body: { reason: options.reason } });
    }
    async deleteServerMember(options) {
        this.handler.request({ method: 'DELETE', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/members/${options.userID}`, body: { reason: options.reason } });
    }
    async getServerBans(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/bans` });
        const collection = new Collection_1.default();
        for (const object of data) {
    ...
    async getServerBan(options) {
        return new Server_1.Ban({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/bans/${options.userID}` }) });
    }
    async createServerBan(options) {
        this.handler.request({ method: 'PUT', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/bans/${options.userID}`, body: { delete_message_days: options.options.deleteMessageDays, reason: options.options.reason } });
    }
    async deleteServerBan(options) {
        this.handler.request({ method: 'DELETE', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/bans/${options.userID}`, body: { reason: options.reason } });
    }
    async getServerRoles(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/roles` });
        const collection = new Collection_1.default();
        for (const object of data) {
            const subject = new Server_1.Role({ client: this, ...object });
            collection.set(subject.flake.id, subject);
        }
        return collection;
    }
    async createServerRole(options) {
        return new Server_1.Role({ client: this, ...await this.handler.request({ method: 'POST', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/roles`, body: { name: options.options.name, permissions: options.options.permissions, color: options.options.color, hoist: options.options.hoist, mentionable: options.options.mentionable, reason: options.reason } }) });
    }
    async updateServerRolePositions(options) {
        const data = await this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/roles`, body: { id: options.options.id, position: options.options.position } });
        const collection = new Collection_1.default();
        for (const object of data) {
            const subject = new Server_1.Role({ client: this, ...object });
            collection.set(subject.flake.id, subject);
    ...
    async updateServerRole(options) {
        return new Server_1.Role({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/roles/${options.roleID}`, body: { name: options.options.name, permissions: options.options.permissions, color: options.options.color, hoist: options.options.hoist, mentionable: options.options.mentionable, reason: options.reason } }) });
    }
    async deleteServerRole(options) {
        this.handler.request({ method: 'DELETE', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/roles/${options.roleID}`, body: { reason: options.reason } });
    }
    async getServerPruneCount(options) {
        this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/prune`, body: { days: options.options.days, include_roles: options.options.includeRoles } });
    }
    async beginServerPrune(options) {
        this.handler.request({ method: 'POST', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/prune`, body: { days: options.options.days, compute_prune_count: options.options.computePruneCount, include_roles: options.options.includeRoles, reason: options.reason } });
    }
    async getServerVoiceRegions(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/regions` });
        const collection = new Collection_1.default();
        for (const object of data) {
    ...
    async getServerInvites(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/invites` });
        const collection = new Collection_1.default();
        for (const object of data) {
            const subject = new Invite_1.InviteMetadata({ client: this, ...object });
            collection.set(subject.code, subject);
    ...
    async getServerIntegrations(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/integrations`, body: { include_applications: options.options.includeApps } });
        const collection = new Collection_1.default();
        for (const object of data) {
    ...
    async createServerIntegration(options) {
        this.handler.request({ method: 'POST', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/integrations`, body: { type: options.options.type, id: options.options.id, reason: options.reason } });
    }
    async updateServerIntegration(options) {
        this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/integrations/${options.integrationID}`, body: { expire_behavior: options.options.expireBehavior, expire_grace_period: options.options.expireGracePeriod, enable_emoticons: options.options.enableEmoticons, reason: options.reason } });
    }
    async deleteServerIntegration(options) {
        this.handler.request({ method: 'DELETE', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/integrations/${options.integrationID}`, body: { reason: options.reason } });
    }
    async syncServerIntegration(options) {
        this.handler.request({ method: 'POST', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/integrations/${options.integrationID}/sync` });
    }
    async getServerWidgetSettings(options) {
        return new Server_1.ServerWidget({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/widget` }) });
    }
    async updateServerWidget(options) {
        return new Server_1.ServerWidget({ client: this, ...await this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/widget`, body: options }) });
    }
    async getServerWidget(options) {
        this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/widget.json` });
    }
    async getServerVanityURL(options) {
        return new Invite_1.default({ client: this, ...await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/vanity-url` }) });
    }
    async getServerWidgetImage(options) {
        return await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/widget.png`, body: { style: options.options.style } });
    }
    ...
    async leaveServer(options) {
        this.handler.request({ method: 'DELETE', url: `${this.options.defaultURLs.base}/users/${options.userID === undefined ? '@me' : options.userID}/guilds/${options.serverID}` });
    }
    ...
    async getServerWebhooks(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/webhooks` });
        const collection = new Collection_1.default();
        for (const object of data) {
            const subject = new Webhook_1.default({ client: this, ...object });
            collection.set(subject.flake.id, subject);
