...
class Client extends Base_1.default {
    ...
    async updateServerRole(options) {
        return new Server_1.Role(await this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/roles/${options.roleID}`, body: { name: options.options.name, permissions: options.options.permissions, color: options.options.color, hoist: options.options.hoist, mentionable: options.options.mentionable, reason: options.reason } }));
    }
    ...
    async getServerIntegrations(options) {
        const data = await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/integrations`, body: { include_applications: options.options.includeApplications } });
        const collection = new Collection_1.default();
        for (const object of data) {
    ...
    async getServerWidgetSettings(options) {
        return new Server_1.ServerWidget(await this.handler.request({ method: 'GET', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/widget` }));
    }
    async updateServerWidget(options) {
        return new Server_1.ServerWidget(await this.handler.request({ method: 'PATCH', url: `${this.options.defaultURLs.base}/guilds/${options.serverID}/widget`, body: options }));
    }
