    ...
    send(op) {
        switch (op) {
            case 1:
                ...
                break;
            case 2:
                this.trySendWS({ op, d: { token: this.client.options.token, properties: this.client.options.properties, compress: this.client.options.isCompress, large_threshold: this.client.options.largeThreshold, shard: [this.id, this.client.options.shard], presence: this.client.options.presence, guild_subscriptions: this.client.options.isServerSubscriptions, intents: this.client.options.intents } });
                break;
        }
    }
            ...
            case 0:
                switch (payload.t) {
                    ...
                    case 'READY':
                        ...
                        this.client.user = new User_1.default({ client: this.client, ...payload.d.user });
                        if (payload.d.guilds !== undefined) {
                            const data = payload.d.guilds;
                            const collection = new Collection_1.default();
                            if (Array.isArray(data))
                                for (const object of data) {
                                    const subject = new Server_1.default({ client: this.client, ...object });
                                    collection.set(subject.flake.id, subject);
                                }
                            ;
                            this.client.servers = collection;
                        }
                        ;
                        break;
                    case 'RESUMED':
                        break;
                    case 'CHANNEL_CREATE': {
                        const channel = new Channel_1.default({ client: this.client, ...payload.d });
                        if (channel.flake)
                            this.client.channels.set(channel.flake.id, channel);
                        this.client.handler.say('channelCreate', channel);
                        break;
                    }
                    case 'CHANNEL_UPDATE': {
                        const channel = new Channel_1.default({ client: this.client, ...payload.d });
                        if (channel.flake)
                            this.client.channels.set(channel.flake.id, channel);
                        this.client.handler.say('channelUpdate', channel);
                        break;
                    }
                    case 'CHANNEL_DELETE': {
                        const channel = new Channel_1.default({ client: this.client, ...payload.d });
                        if (channel.flake)
                            this.client.channels.delete(channel.flake.id);
                        this.client.handler.say('channelDelete', channel);
                        break;
                    }
                    case 'CHANNEL_PINS_UPDATE': {
                        this.client.handler.say('channelPinsUpdate', payload.d);
                        break;
                    }
                    case 'GUILD_CREATE': {
                        let server = new Server_1.default({ client: this.client, ...payload.d });
                        // (server.flake.id >> 22n) % BigInt(this.client.options.shard);
                        server.channels.forEach(channel => this.client.channels.set(channel.flake.id, channel));
                        this.client.servers.set(server.flake.id, server);
                        this.client.handler.say('serverCreate', server);
                        break;
                    }
                    case 'GUILD_UPDATE':
                        this.client.handler.say('serverUpdate', new Server_1.default({ client: this.client, ...payload.d }));
                        break;
                    case 'GUILD_DELETE':
                        this.client.handler.say('serverDelete', new Server_1.default({ client: this.client, ...payload.d }));
                        break;
                    case 'GUILD_BAN_ADD':
                        this.client.handler.say('serverBanAdd', payload.d);
                        break;
                    case 'GUILD_BAN_REMOVE':
                        this.client.handler.say('serverBanRemove', payload.d);
                        break;
                    ...
                    case 'INVITE_DELETE':
                        break;
                    case 'MESSAGE_CREATE': {
                        const message = new Message_1.default({ client: this.client, ...payload.d });
                        this.client.channels.get(message.channelFlake.id).lastMessageFlake = message.flake;
                        this.client.handler.say('messageCreate', message);
                        break;
                    }
                    case 'MESSAGE_UPDATE': {
                        const message = new Message_1.default({ client: this.client, ...payload.d });
